...
export interface DeceasedInfo {
  ...
  /**
   * صورة الفقيد كـ data URL (base64). تُعرض دائماً داخل إطار ثابت طولي ٧×١٠سم
   * (object-fit: cover — لا contain) بصرف النظر عن اتجاه الصورة الأصلية (عرضية
   * أو طولية)، مع تحكّم المستخدم بالتكبير/التحريك داخل الإطار عبر photoCrop.
   * راجع PhotoUpload.tsx (التعديل التفاعلي) وObituaryBlocks.tsx (العرض النهائي)
   * وlib/obituary/photoCrop.ts (الحساب المشترك).
   *
   * ⚠️ **لم تعد هذه الصورة محصورة بالمتصفح** منذ إضافة أرشفة Google Drive: عند كل
   * تصدير فعلي (PNG/PDF/مشاركة)، تُرسَل صورة النعوة الكاملة (وهذه الصورة مدموجة
   * فيها) إلى /api/archive ثم إلى Drive — راجع lib/export/archive.ts. لا تُعِد
   * كتابة هذا التعليق إلى "لا تُرسل لأي خادم" بلا إزالة تلك الأرشفة فعلياً أولاً.
   */
  photoDataUrl?: string
  /**
  ...
  photoCrop?: PhotoCrop
  /**
   * مضاعف حجم إطار الصورة على الصفحة (زر +/- بخطوات ١٠٪ في PhotoUpload.tsx) —
   * منفصل تماماً عن PhotoCrop.zoom (ذاك يكبّر محتوى الصورة داخل إطار ثابت، هذا
   * يكبّر الإطار نفسه). راجع PHOTO_SIZE_SCALE_* في lib/obituary/photoCrop.ts.
   */
  photoSizeScale?: number
  /**
   * تخطيط الصورة: **معطَّل افتراضياً** (undefined يُعامل كـfalse — طُلب صراحةً أن
   * تكون الصورة متوسِّطة فوق الاسم افتراضياً، لا جانب النص) — الصورة جهة اليسار
   * والنص (جملة النعي + سطر الترحّم + المرحوم + الاسم) جهة اليمين في صفّ واحد،
   * بدل الاستهلاك الرأسي للمساحة الذي يُفعِّل تصغير auto-fit بسرعة أكبر. تفعيله
   * يستبدل التخطيط المتوسِّط/المكدَّس الرأسي الافتراضي. راجع ObituaryBlocks.tsx.
   */
